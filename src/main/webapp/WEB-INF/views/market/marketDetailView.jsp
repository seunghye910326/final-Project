         ...
         <div class="container px-4 px-lg-5">
            ...
            <div class="row gx-4 gx-lg-5 align-items-center my-5">
                <div class="col-lg-6">
                    <div class="fotorama">
                        <c:forEach items="${ at }" var="at" >
                            <div>
                                <img src="${at.changeName}" style="width: 100%; height: 350px;" />
                            </div>
                        </c:forEach>
                    </div>
                </div>
                ...
                <div id="marketDetalcontent" class="col-lg-6">
                    <h2 class="font-weight-bold"> ${ m.marketTitle }
                        <c:choose>
                    ...
                    <c:if test="${ loginUser.memNo == m.memNo }" >
                    ...
                    <form action="" method="post" id="postForm">
                        <input type="hidden" name="marketNo" value="${ m.marketNo }" />
                    </form>
            ...
            <table id="replyArea" class="table" align="center">
                <thead>
                    ...
                    <tr>
                        <td style="padding: 10px;" colspan="7">댓글(<span id="rcount">0</span>)</td>
                    </tr>
                </thead>
                <tbody>
                    <!--
        ...
        <script>
                ...
                $(function(){
                                    ...
                                    if(cList.length != 0) { //댓글 리스트가 조회된 경우
                                        ...
                                        for(let i in cList){
                                            ...
                                            if(cList[i].secret==1){ //비밀글로 설정한 경우 
                                                ...
                                                if(cList[i].memId=='${loginUser.memId}' || '${m.memNo}'=='${loginUser.memNo}'){ 

                                                    //비밀글이면서 댓글작성자이거나 게시글작성자인 경우
                                                    value += '<tr>'
                                                    + '<th colspan="2">' + cList[i].memId + '</th>'
                                                    + '<th colspan="2">' + cList[i].commentContent + '</th>'
                                                    + '<th>' + cList[i].commentEnrollDate + '</th>';
                                                    
                                                    
                                                    if(cList[i].memId=='${loginUser.memId}') {//댓글작성자인 경우
                                                        value +=  '<th>'
                                                            +   '<button class="btn btn-sm" onclick="deleteComment(' 
                                                            +   cList[i].commentNo
                                                            +   ')"> X </button>'
                                                            +   '</th>'
                                                            +   '<th>'
                                                            +   '<button class="btn btn-sm updateComment" onclick="updateComment('
                                                            +   cList[i].commentNo + ','
                                                            +   ')"> 수정 </button>'
                                                            +   '</th>'
                                                        ...
                                                        value += '<tr>'
                                                            + '<th colspan="7" class="text-center">' + '비밀댓글입니다.'
                                                            + '</th>'
                                                            + '</tr>';
                                                    }
                                                    ...
                                                    value += '<tr>'
                                                        + '<th>' + cList[i].memId + '</th>'
                                                        + '<th colspan="3">' + cList[i].commentContent + '</th>'
                                                        + '<th style="width:150px;">' + cList[i].commentEnrollDate + '</th>';
                                                        
                                                        //로그인한 멤버가 댓글작성자인 경우 수정과 삭제 버튼 띄워주기
                                                        if(cList[i].memId=='${loginUser.memId}') { 

                                                            value +=  '<th style="width:20px;">'
                                                                +   '<button class="btn btn-sm" onclick="deleteComment(' 
                                                                +   cList[i].commentNo 
                                                                +   ')"> X </button>'
                                                                +   '</th>'
                                                                +   '<th style="width:20px;" data-cno="' 
                                                                +   cList[i].commentNo 
                                                                +   '">'
                    ...
                    $('#replyArea tbody').on('click', '.updateComment', function(){ //동적으로 생성된 요소에 이벤트 부여
                        ...
                        var th = $(this).parent(); //수정버튼을 클릭한 요소에서 제일 가까운 th를 찾아오기
                        cno = th.data("cno");
                        ...
                        $('#modal #commentContent').on('keyup', function(){ //textarea keyup이벤트 실행시
